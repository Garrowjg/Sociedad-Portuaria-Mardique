...
function buildGraphUrl(path) {
    return GRAPH_BASE + "/sites/" + SHAREPOINT_SITE_ID + path;
}
...
async function getCurrentUser() {
    ...
    try {
        const data = await graphFetch(GRAPH_BASE + "/me?$select=displayName,mail,jobTitle,department,officeLocation,userPrincipalName");
        return data;
    } catch (e) {
        console.warn("getCurrentUser: error Graph API, usando mock:", e.message);
        await mockDelay();
        return MOCK_USER;
    }
}
...
async function getDriveRoot() {
    ...
    try {
        ...
        console.warn("getDriveRoot: error Graph API, usando datos locales:", e.message);
        await mockDelay();
        return getMockDriveRoot();
    }
}
...
async function getDriveItems(folderId) {
    ...
    try {
        ...
        console.warn("getDriveItems: error Graph API, usando datos locales:", e.message);
        await mockDelay();
        return getMockDriveItems(folderId);
    }
}
...
async function submitLeaveRequest(formData) {
    ...
    const data = await graphFetch(GRAPH_BASE + "/sites/" + SHAREPOINT_SITE_ID + "/lists/SolicitudesVacaciones/items", {
        method: "POST",
...
async function submitSupportTicket(formData) {
    ...
    const data = await graphFetch(GRAPH_BASE + "/sites/" + SHAREPOINT_SITE_ID + "/lists/SoporteTecnico/items", {
        method: "POST",
        body: JSON.stringify(body)
    });
    return data;
...
async function getAreas() {
    try {
        ...
        console.warn("getAreas: error Graph API, usando datos locales:", e.message);
        await mockDelay();
...
async function submitConversation(formData) {
    ...
    const data = await graphFetch(GRAPH_BASE + "/sites/" + SHAREPOINT_SITE_ID + "/lists/Conversaciones/items", {
        method: "POST",
        body: JSON.stringify(body)
    });
    return data;
...
function renderDocumentsWithFilter(items, containerId) {
    ...
    if (currentSector !== "todos") {
        if (MODO_PRUEBA) {
            ...
            filtered = items.filter(i => i.name && i.name.toLowerCase().includes(currentSector));
        }
    }
